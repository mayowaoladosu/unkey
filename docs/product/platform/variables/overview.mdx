...
### Rename or delete across every environment

Variables with the same name in more than one environment are shown as a single group. The group's action menu lets you:

- **Rename**: change the key across every environment where the variable exists in one step. Duplicate keys within the target environments are rejected up front.
- **Delete all**: remove every occurrence of the variable across environments.

Renames and deletes take effect on the next deployment, so trigger a redeploy to apply the change.
