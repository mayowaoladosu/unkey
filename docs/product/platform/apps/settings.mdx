...
Override the build command Unkey uses for [automatic builds](/builds/overview). Leave empty to let Unkey detect the command automatically. Useful for monorepos or when you need to build a specific package, for example:

```bash
pnpm build --filter api
```

The setting only applies to automatic builds. When a [Dockerfile](#dockerfile) is configured, the build follows the Dockerfile and this field is ignored. Changes take effect on the next deployment.
...
The server path where your application serves its OpenAPI specification (for example, `/openapi.yaml` or `/api/spec.json`). When set, Unkey fetches the spec from your running deployment for use with [OpenAPI validation](/platform/gateway/policies/openapi-validation) and API documentation.
