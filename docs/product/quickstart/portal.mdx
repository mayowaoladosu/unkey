...
Sessions granted `read_analytics` see verification analytics scoped to the session's `externalId`. Wildcard filters are safe to use: the gateway automatically restricts every query to the session's identity, so a portal user only sees their own verifications even when the underlying dashboard could show more.

Deleted keys are included so historical charts stay complete after a key is revoked.

## Audit logging

Key mutations initiated by a portal session — create, delete, and reroll — are recorded in the workspace [audit log](/audit-log/introduction). The actor is the portal user, identified by the session's `externalId`, which lets you attribute changes back to the end user who made them. Filter the audit log by actor type to see only portal-driven changes.
