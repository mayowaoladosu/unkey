...
func (w *Workflow) WakeDeployment(ctx restate.ObjectContext, req *hydrav1.WakeDeploymentRequest) (*hydrav1.WakeDeploymentResponse, error) {
	...
	if err := w.wakeStoppedDeployment(ctx, deployment); err != nil {
		return nil, err
	}
	return &hydrav1.WakeDeploymentResponse{}, nil
}

func (w *Workflow) wakeStoppedDeployment(ctx restate.ObjectContext, deployment db.Deployment) error {
	...
	if err != nil {
		return fmt.Errorf("wake deployment workflow failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("mark waking deployment as deploying: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("find wake topology min replicas: %w", err)
	}
	if len(rows) == 0 {
	...
	if err != nil {
		return fmt.Errorf("mark woken deployment ready: %w", err)
	}
	return nil
}
