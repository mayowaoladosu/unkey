...
//
//	SELECT pk, id, workspace_id, project_id, app_id, environment_id, kind, target_key, deployment_id, previous_deployment_id, created_at, updated_at FROM deployment_targets WHERE id = ?
func (q *Queries) FindDeploymentTargetByID(ctx context.Context, id string) (DeploymentTarget, error) {
	row := q.db.QueryRowContext(ctx, findDeploymentTargetByID, id)
...
//	  AND kind = ?
//	  AND target_key = ?
func (q *Queries) FindDeploymentTargetByIdentity(ctx context.Context, arg FindDeploymentTargetByIdentityParams) (DeploymentTarget, error) {
	row := q.db.QueryRowContext(ctx, findDeploymentTargetByIdentity,
		arg.AppID,
		arg.EnvironmentID,
		arg.Kind,
		arg.TargetKey,
