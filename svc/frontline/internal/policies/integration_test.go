...
func TestRateLimit_RemoteIP(t *testing.T) {
	...
	policies := []*frontlinev1.Policy{
		rateLimitPolicy("rl", 2, 60000, &frontlinev1.RateLimitIdentifier{
			Source: &frontlinev1.RateLimitIdentifier_RemoteIp{
...
func TestRateLimit_AuthenticatedSubject(t *testing.T) {
	...
	policies := []*frontlinev1.Policy{
		...
		rateLimitPolicy("rl", 1, 60000, &frontlinev1.RateLimitIdentifier{
...
func TestRateLimit_PrincipalField(t *testing.T) {
	...
	policies := []*frontlinev1.Policy{
		keyAuthPolicy("auth", []string{s1.KeySpaceID, s2.KeySpaceID}),
		rateLimitPolicy("rl", 1, 60000, &frontlinev1.RateLimitIdentifier{
			Source: &frontlinev1.RateLimitIdentifier_PrincipalField{
				PrincipalField: &frontlinev1.PrincipalFieldKey{Path: "subject"},
			},
		}),
	}
...
func TestRateLimit_NoPrincipal(t *testing.T) {
	...
	policies := []*frontlinev1.Policy{
		rateLimitPolicy("rl", 10, 60000, &frontlinev1.RateLimitIdentifier{
			Source: &frontlinev1.RateLimitIdentifier_AuthenticatedSubject{
				AuthenticatedSubject: &frontlinev1.AuthenticatedSubjectKey{},
			},
		}),
