...
export function SidebarBody() {
	...
	const keyAuthId = useApiKeyAuthId(
		context.type === "api" ? context.apiId : undefined,
	);
	const appOverview = useFlag("appOverview");
	const portalManagement = useFlag("portalManagement");
	const deploymentId = pathname.match(/\/deployments\/([^/]+)/)?.[1];

	const links = (() => {
		if (context.type === "project" && context.appId && deploymentId) {
