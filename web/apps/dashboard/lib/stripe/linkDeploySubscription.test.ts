...
const h = vi.hoisted(() => {
  ...
  const invalidateWorkspaceCache = vi.fn();
  return { where, set, update, findFirst, transaction, insertAuditLogs, invalidateWorkspaceCache };
});
...
describe("linkDeploySubscription", () => {
  beforeEach(() => {
    ...
    h.invalidateWorkspaceCache.mockClear();
    h.update.mockClear();
  });
  ...
  it("writes customer + subscription + plan for a paid, active, unlinked workspace", async () => {
    ...
    expect(h.invalidateWorkspaceCache).toHaveBeenCalledWith("org_1");
  });
