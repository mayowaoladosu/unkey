...
export const getDeploymentRuntimeLogs = workspaceProcedure
	...
	.query(async ({ ctx, input }) => {
		const deployment = await db.query.deployments.findFirst({
			where: (table, { and, eq }) =>
				and(
					eq(table.id, input.deploymentId),
		...
		const { logsQuery } = await clickhouse.runtimeLogs.logs(
			{
				...
				resourceId: input.resourceId ? [input.resourceId] : [],
				resourceKind: [],
		...
		return {
			logs: chLogs.map((log) => ({
				...
				resourceKind: log.resource_kind,
				instance_id:
					k8sNameToInstanceId.get(
						toInstanceKey(log.k8s_pod_name, log.region),
					) ?? "—",
				region: log.region,
			})),
		};
