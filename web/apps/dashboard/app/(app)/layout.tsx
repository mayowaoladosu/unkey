...
import {
	SIDEBAR_WIDTH_VARS,
	SidebarV2,
} from "@/components/navigation/sidebar-v2";
import {
	MobileNavDrawer,
	MobileNavTrigger,
} from "@/components/navigation/sidebar-v2/mobile-nav-drawer";
	...
	return (
		<div className="fixed top-0 inset-x-0 z-50 flex justify-center border-t-2 border-error-9">
			<div className="bg-error-9 flex -mt-1 font-mono items-center gap-2 text-white text-xs rounded-b overflow-hidden shadow-lg select-none pointer-events-none px-1.5 py-0.5">
				Impersonation Mode. Do not change anything and log out after you are
				done.
			</div>
		</div>
	);
...
export default function Layout({ children }: LayoutProps) {
	...
	const { user, workspace, isLoading, error, workspaceMissing } =
		useWorkspace();
	...
	return (
		<SidebarProvider style={SIDEBAR_WIDTH_VARS}>
			<div className="h-dvh w-full flex overflow-hidden bg-white dark:bg-base-12">
				<MobileNavDrawer />
				{!isAppOnboarding ? <MobileNavTrigger /> : null}
				{!isAppOnboarding && (
