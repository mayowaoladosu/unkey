      ...
      <Card className="overflow-hidden">
        {resources.isPending ? (
          ...
          <div className="p-4 text-xs text-error-11">
            Deployment resources could not be loaded. The deployment remains available.
          </div>
        ) : (
            ...
            >
              {resources.data.manifest ? (
                <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                  <ResourceValue label="Adapter" value={resources.data.manifest.adapterId} />
                  <ResourceValue label="Output" value={resources.data.manifest.outputMode} />
                  <ResourceValue
                    label="Schema"
                ...
                {resources.data.artifacts.map((artifact) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-gray-12">{artifact.name}</p>
                      <p className="truncate text-[11px] text-gray-10">
                        {artifact.kind.replaceAll("_", " ")} · {formatBytes(artifact.sizeBytes)} ·{" "}
              ...
              <div className="grid gap-2">
                {resources.data.aliases.map((alias) => (
                  ...
                  >
                    ...
                    <ResourceBadge>{alias.mutable ? alias.sticky : "immutable"}</ResourceBadge>
                  </div>
                ))}
              </div>
            </ResourceBlock>
                    ...
                    {resources.data.history.slice(0, 5).map((assignment) => (
                      ...
                      >
                        <span className="text-gray-11">
                          {assignment.targetKind}:{assignment.targetKey} · {assignment.reason}
                        </span>
  ...
  return (
    <div className="rounded-md border border-grayA-4 bg-grayA-2 px-3 py-2">
      ...
      <div className="mt-1 flex items-center gap-1">
        ...
        {copyValue ? <CopyButton value={copyValue} variant="ghost" toastMessage={label} /> : null}
      </div>
    </div>
  );
}

function ResourceBadge({ children, accent = false }: { children: ReactNode; accent?: boolean }) {
  return (
    <span
      className={
        accent
