...
import {
	type Deployment,
	deploymentSchema,
} from "@/lib/collections/deploy/deployments";
...
const DeploymentLayoutContext =
	createContext<DeploymentLayoutContextType | null>(null);
...
export const DeploymentLayoutProvider = ({
	...
	if (!deploymentId) {
		throw new Error(
			"DeploymentLayoutProvider requires a deploymentId (via prop or route params)",
		);
	}

	const { getDeploymentById, isDeploymentsLoading, projectId } =
		useProjectData();
	const deployment = getDeploymentById(deploymentId);

	const { data: fetchedDeployment, isLoading: isFetchingById } =
		trpc.deploy.deployment.getById.useQuery(
			{ deploymentId, projectId },
			{ enabled: !deployment },
		);

	const parsed = fetchedDeployment
		? deploymentSchema.safeParse(fetchedDeployment)
		: undefined;
