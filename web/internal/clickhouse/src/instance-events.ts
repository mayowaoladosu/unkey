...
export function getInstanceEvents(ch: Querier) {
	return async (args: InstanceEventsRequest) => {
      ...
      AND (
        CASE WHEN length({resourceIds: Array(String)}) > 0
          THEN resource_id IN {resourceIds: Array(String)}
          ELSE TRUE
        END
      )
      AND (
        CASE WHEN length({resourceKinds: Array(String)}) > 0
          THEN resource_kind IN {resourceKinds: Array(String)}
          ELSE TRUE
        END
      )
      AND (
